...
The TUI source lives in `src/tui/`; do not edit `dist/tui.mjs` directly. Ink and React are development dependencies bundled into that generated artifact so the published CLI keeps zero required runtime dependencies. TUI tests should exercise rendered frames and keyboard input through `ink-testing-library`, including preview-confirm flows, focused direct actions, cancellation, input locking, result refresh, and failed concurrent writes for state-changing flows. Collection displays, daily narrative interactions, and Expeditions also need coverage proving their limits, stable-ID persistence, historical-date filtering, non-rerollable plans, non-stacking opportunities, return presentation, and lack of Token-volume rewards. Read-only historical projections such as Chronicle and collection sets need selected-date, privacy, no-state-creation, bilingual, and stable-public-JSON coverage. Living Habitat changes must keep route pools balanced and verify that terminal, TUI, JSON, SVG, motion-off, and 80-column views consume the same deterministic scene semantics.
