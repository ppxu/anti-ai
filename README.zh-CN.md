...
每个活跃日都会获得使用特征加点、一次确定性随机加点和事件关联奖励。能力上限为 `99`，达到 `5`、`15`、`30` 时会依次解锁越来越不妙的畸变天赋。失控指数每 10 点让稀有突变率增加 1 个百分点，从基础 `8%` 最高升到 `20%`。等级、性格、心情、称号、活跃连击、孵化年龄、天赋收藏和今日加点都会出现在终端以及 `creature --json` 中。

累计污染达到 `50`、`150`、`350` 时进入后续阶段。每天会根据本地随机种子和日期触发一个可复现事件。首个活跃日之后，每个 AI 清醒日让污染减少 `2`、戒断反应增加 `1`，怪兽进入休眠，但历史性状不会被清空。

档案保存在 `~/.anti-ai/creature.json`。文件只包含污染剂量、性状、能力加点、事件 ID 和本地随机种子，不包含 Prompt、回复、路径、模型名、精确 Token 或逐请求时间。已有的 v0.5 档案会在首次运行时在本地自动升级。成长档案固定使用 Codex + Claude Code 完整数据，不能配合 `--source` 过滤。
