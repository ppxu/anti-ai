...
固定收藏共 134 项：16 个形态家族、36 个成就、12 个异色能力、4 种世代伤痕、30 个路线对等的生态现象、24 件远征遗物和 12 项远征成就。人类可读输出只揭示已发现名称，锁定项保持 `???`；动态标本指纹、外来遭遇标本、永久化石、已封存病例切片、污染培养物、已绑定伴生形态和已结案事故报告则不设人为上限。另有六组路线对等的病理套组从现有发现派生，不计入固定分母；完成只解锁展示语料。

`codex --json` 提供稳定 ID、发现状态与日期、来源链、收藏计数、派生套组进度、指定日期的 `recent` 新发现，以及当前 3 个陈列位引用。TUI 提供分类、条目和详情浏览、首次发现与关联记录，以及内嵌的 7/30 天“收容档案”；陈列动作必须明确确认。陈列只改变图鉴、生态舱和分享展示。图鉴与 `creature` 共用完整的六来源成长史，因此拒绝 `--source` 过滤，也不会把多烧 Token 变成首选收集路线。套组条件与纵向记录见[异变年鉴指南](./docs/chronicle.zh-CN.md)。
...
核能巨兽生成器包含 16 个核心形态家族，以及 **204,374,016 种去重后的最终 ASCII 形象**。本地稳定基因决定器官，使用病型、生态人格、伤疤、成就、异色稀有度和已封存世代嫁接器官则继续改写同一条骨架。运行 `anti-ai codex` 可以对照理论容量与个人收藏。
