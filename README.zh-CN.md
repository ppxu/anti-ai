...
机会不累计。跳过没有惩罚，但结算新的阅历日后，旧机会会过期；退出只暂停当前进度，放弃则消耗机会，同时保留已经发生的事件、遗物和永久微调。人类输出会把已揭示格明确标为静默格、普通事件、状态变动或特殊事件，并生成与 TUI、远征分享卡一致的返航总结。图鉴新增 24 件固定目的地遗物与 12 项远征成就，均不提供战力、分数、更多远征或 Token 奖励。完整事件牌组、TUI 操作、收藏、档案和隐私边界见[收容远征](./docs/expeditions.zh-CN.md)。[English](./docs/expeditions.md)。
