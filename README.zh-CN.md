...
固定收藏共 134 项：16 个形态家族、36 个成就、12 个异色能力、4 种世代伤痕、30 个路线对等的生态现象、24 件远征遗物和 12 项远征成就。人类可读输出只揭示已发现名称，锁定项保持 `???`；动态标本指纹、外来遭遇标本、永久化石、已封存病例切片、污染培养物、已绑定伴生形态和已结案事故报告则不设人为上限。另有 12 组路线对等的病理星图从现有发现派生，不计入固定分母；每条路线含 1 个稀有征象、2 个史诗综合征和 1 个隐藏传说复合诊断，四阶段进度都只解锁展示语料。

`codex --json` 提供稳定 ID、发现状态与日期、来源链、收藏计数、派生星图进度、馆藏异变状态、指定日期的 `recent` 新发现，以及当前 3 个陈列位引用。`codex --set <set-id>` 可聚焦一组证据星图，同时保持 JSON 不受语言影响。TUI 提供分类、条目和详情浏览、首次发现与关联记录，以及内嵌的 7/30 天“收容档案”；陈列动作必须明确确认。陈列只改变图鉴、生态舱和分享展示。图鉴与 `creature` 共用完整的六来源成长史，因此拒绝 `--source` 过滤，也不会把多烧 Token 变成首选收集路线。星图条件与纵向记录见[异变年鉴指南](./docs/chronicle.zh-CN.md)。

人类可读图鉴会分开展示两种兼容口径：**204,374,016 种去重基础标本形态**继续作为指纹与污染码的身份空间；`无表型 + 7 种证据母题 × 4 个里程碑`形成 29 种只读馆藏表型和 **5,926,846,464 种理论展示形态**。固定馆藏只有在 34/67/101/134 项且覆盖 3/5/6/7 类时才触发对应阶段；它们只改变展示，不增加数值或改写身份。容量算法与视觉覆盖优先级详见[异变体成长指南](./docs/creature.zh-CN.md)。

### `expedition`

每个本地自然日最多开启一次十格收容远征，不需要等待异变体结算：
...
机会不累计。跳过没有惩罚；档案进入更新的结算日期或远征日期后，旧日期机会会过期。退出只暂停当前进度；如果隔天才完成或放弃旧远征，当天的新机会仍然可用。放弃会保留已经发生的事件、遗物和永久微调。人类输出会把已揭示格明确标为静默格、普通事件、状态变动或特殊事件，并生成与 TUI、远征分享卡一致的返航总结。图鉴新增 24 件固定目的地遗物与 12 项远征成就，均不提供战力、分数、更多远征或 Token 奖励。完整事件牌组、TUI 操作、收藏、档案和隐私边界见[收容远征](./docs/expeditions.zh-CN.md)。[English](./docs/expeditions.md)。
...
成长史与实验室现在支持 11 种隐私安全卡片：`pathology` 是病理切片，`specimen` 是当前收藏标本，`wanted` 是讽刺悬赏令，`fossil` 是最近一代的永久化石证书，`encounter` 是本地接触事故卡，`prognosis` 是当前病例的三路线预演，`culture` 是已封存培养事故，`companion` 是当前绑定伴生异物的成长档案，`habitat` 是双体收容场景，`expedition` 是当前或最近一局有效远征，`dossier` 汇总当前年鉴、30 天病程、世代对照和套组印章。化石证书会在第 90 个阅历日后开放；预后卡会在转折病例待处理时开放；培养物卡默认选择最近一份，也可使用 `--id` 指定；伴生卡会在执行 `lab bond` 后开放。生态舱卡在伴生位空置时也可以生成；远征卡查看历史日期时不会暴露未来记录。

工具不会上传卡片；所有卡片均不包含精确 Token、请求数、来源/模型名、路径或对话正文。CLI 始终写入标准输出，保存位置由命令行重定向决定；TUI 确认导出时优先使用启动目录，不可写时自动改存 `~/.anti-ai/exports`。异变体卡片必须使用完整数据，因此会拒绝 `--source` 过滤。
...
每 14 个阅历日最多出现一个转折病例。污染、清醒、悖论三条路线都会明示收益和代价；病例未处理时不会继续堆积选择。`history` 压缩关键成长节点，`chronicle` 只读对照 7/30/90 天与不同世代，`prognosis` 使用定性标签预演三个可解释方向，不伪造精确概率。详见[异变年鉴指南](./docs/chronicle.zh-CN.md)。
