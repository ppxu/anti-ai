...
├──────────────────────────────────────────────┤
  ...
  生活翻译（终于像人话了）
  💡  10W LED 灯       25.39–35.97 小时
  🚰  550mL 矿泉水     0.50–98.21 瓶
  🚗  平均燃油车        0.13–5.51 公里
...
打印指定自然日的 Token 用量、来源拆分和公开资源代理跨度。日期按系统本地时区计算。

`--json` 只输出可精确核对的 Token 统计，不把低置信度资源估算混入机器数据。
...
树种、树龄和砍伐后的处理方式会显著影响碳排，因此工具不显示缺乏依据的“砍了几棵树”，而是显示“一棵城市树需要多久才能吸收对应碳排”。
