...
Opening, navigating, inspecting, replaying, or cancelling inside `anti-ai tui` does not scan Agent records or write state. Entering the daily-settlement impact preview may scan supported usage metadata, but it still does not write. Consequence Cabinet changes and the once-per-settled-day Observation and Contact actions write only stable collection, target, and reaction IDs after explicit confirmation; they grant no numeric reward. Every write goes through the shared application service and the same validated, atomic, optimistic-concurrency state store as the CLI. Ink and React are bundled locally; the console makes no network request and starts no background service.
