...
这个独立 Swift/AppKit Package 是 anti-ai v4.0 的正式原生展示适配器。它位于 npm
...
150 × 140 pt 的异变体默认可以拖动。菜单栏图标可控制显示、位置锁定/重置、四种展示
状态、三档动态、中英文切换、快照刷新、打开完整 TUI 和退出。屏幕休眠时会暂停，遵循
系统“减少动态效果”，其他应用占据全屏时自动隐藏。菜单也支持手动检查 App 更新，自动
检查默认关闭，只有用户明确开启后才生效。
...
v4.0.0 下载版是明确的临时未公证预览版，只使用 ad-hoc 签名。首次启动被拦截后，可能
...
禁用更新入口，只生成本地正式候选。v4.0 预览分发必须配置 Sparkle Ed25519 密钥、签名
