...
- `codex`, `creature habitat`, `creature chronicle`, every `share` card, `doctor`, `explain`, and Help remain read-only snapshots. Chronicle, generation comparison, collection-set progress, and Living Habitat scenes are derived in memory and add no schema field.
