...
- `codex`、`creature habitat`、`creature chronicle`、所有 `share` 卡片、`doctor`、`explain` 与 Help 仍是只读快照。年鉴、世代对照、套组进度和活体生态舱场景只在内存中派生，不增加 schema 字段。
