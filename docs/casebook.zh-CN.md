...
同时最多保留一个待处理病例。只要它还没选择，后续间隔就不会堆出一串待办。首版内置 12 个确定性病例骨架，由当前使用病型或生态人格决定：
...
schema v1–v11 会在本地逐版本幂等迁移。迁移会保留病历，并在需要时建立空事故、实验室、伴生与后果陈列柜索引，不会虚构历史选择、事故、实验、绑定、陈列或互动。
