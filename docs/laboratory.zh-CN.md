...
已封存培养物也会进入 `anti-ai codex`，记录稳定 ID、类型、稀有度、发现日期、原料类别和外观指纹。它们不会改变固定 68 项图鉴的分母。
...
异变体档案使用 schema v12。Laboratory v2 只保存：
...
schema v1–v11 会在本地逐版本幂等迁移，保留或补充缺失的空实验室、伴生、事故或后果陈列柜索引，不会虚构历史实验、绑定、印记、异常、事故、陈列或互动。
