  ...
  const sections = {
    ...
    creature: {
      zh: [
        ...
        "生态舱是只读共居快照：每 7 个阅历日派生一次生态事件，三条路线拥有同等规模的事件池。",
        "活体生态舱包含 15 种确定性场景原型；重复查看、语言、终端宽度和动态档位都不能重抽。",
        "生态舱只改变场景、关系与叙事，不能改变异变体或伴生物数值。",
        "已保存外来标本可显式入住访客位；共处按自然日派生，不增加成长或 Token 奖励。",
        "每个本地自然日最多开启一局十格收容远征，不依赖成长结算；机会不累计，退出可恢复，放弃不可重抽。",
        "远征次数、格数、事件、永久微调和收藏概率都不随 Token 量增加。",
        "history 压缩关键节点；chronicle 只读对照 7/30/90 天与世代；prognosis 只给可解释方向，不伪造精确概率。",
      ...
      en: [
        ...
        "The read-only containment habitat derives one ecological incident per seven experience days, with equal-sized route pools.",
        "The living habitat contains 15 deterministic scene archetypes; reopening, language, terminal width, and motion cannot reroll them.",
        "The habitat changes scenery, relationships, and narrative; it cannot change creature or companion numbers.",
        "A saved foreign specimen can explicitly enter the visitor bay; natural-date cohabitation adds no growth or Token reward.",
        "Each local calendar day offers at most one ten-cell containment expedition, independent of settlement; opportunities do not stack, exits can resume, and abandonment cannot reroll.",
        "Expedition count, cells, events, permanent adjustments, and collection odds never increase with Token volume.",
        "history compresses key events; chronicle compares 7/30/90-day and generation records read-only; prognosis shows explainable directions with no precise probabilities.",
    ...
    privacy: {
      zh: [
        ...
        "creature habitat、codex 和所有 SVG 分享卡都是只读快照；CLI 卡片只写入标准输出，TUI 确认后只新建预览过的 SVG；当前目录不可写时改存 ~/.anti-ai/exports。",
      ...
      en: [
        ...
        "Creature habitat, Codex, and every SVG card are read-only snapshots; CLI cards use stdout, while confirmed TUI exports create only the previewed SVG and fall back to ~/.anti-ai/exports when the current directory is not writable.",
...
function runExplain(lang = "zh", topic = undefined) {
  ...
  if (lang === "en") {
    const lines = [
      ...
      "  Growth balance v2 uses the median of up to 28 prior non-zero days; AI-free",
      "  days never lower that baseline. High use adds 1–3 POLLUTION, low use adds",
      "  1–2 CLARITY, and an AI-free day adds 3 CLARITY. Dose 75+ applies at least",
      "  1 POLLUTION but never grants an extra ability point or faster stage growth.",
      "  Lifetime POLLUTION and CLARITY remain in history; the latest 28 experience",
      "  days form the current UNFORMED, POLLUTED, LUCID, or PARADOX ecology.",
      "  A candidate must persist for 3 settled days",
      ...
      "  Thirty-six achievements are split evenly across OFFENSE, SOBRIETY, and PARADOX badges;",
      "  repeatable badges grow through three behavior-based tiers without exact Token totals.",
      "  anti-ai codex derives 134 fixed entries plus private, foreign, fossil, selected case, culture, companion, and incident-report specimens from schema v16.",
      "  Twelve route-balanced Pathology Constellations derive from those discoveries; four case phases and concealed Legendary diagnoses unlock presentation only.",
      "  Fixed collection milestones at 34/67/101/134 discoveries can add one of 28 display-only crowns or exoskeletons without changing the base fingerprint.",
      "  Base specimen capacity stays 204,374,016; 29 Collection Phenotypes produce 5,926,846,464 theoretical displayed forms.",
      "  Locked human entries remain ???; JSON exposes stable IDs and discovery dates.",
      "  The TUI can place up to three discovered entries in a consequence cabinet.",
      "  One observation and one contact may be sealed per settled day; both add narrative only.",
      ...
      "  Every 14 experience days may offer at most one turning-point case from 24",
      ...
      "  Every 7 experience days may offer at most one containment incident. Heavy,",
      "  restrained, and AI-free days advance the cadence equally. EMERGENCY QUARANTINE,",
      "  CONTINUE OBSERVATION, and ALLOW RESONANCE each seal a benefit and a cost;",
      "  the delayed aftermath resolves after 3 experience days. Incident responses grant",
      "  no abilities, experience, or Token rewards and an unanswered incident never builds a backlog.",
      "  Each local calendar day offers at most one ten-cell containment expedition;",
      "  eligibility does not depend on settlement; opportunities do not stack, and past dates expire.",
      "  The four destinations seal a deterministic non-rerollable event plan; exits resume it,",
      "  while abandonment consumes the opportunity and preserves reached permanent effects.",
      "  Token volume changes neither run count, cells, event sequence, artifact odds, nor rarity.",
      "  Expeditions add 24 fixed artifacts and 12 fixed achievements to the Codex.",
      "  The Token Metabolic Clinic describes usage-metadata correlations, never productivity,",
      "  causality, personal ability, or health. Relative rules compare with up to 14 active days",
      "  and require at least 3 comparable samples. Its 7-, 14-, and 30-day passive studies",
      "  advance by calendar date; missed days never reset, punish, or extend them.",
      "  Study seals never add abilities, experience, rarity, or collection progress; nor Token rewards.",
      "  anti-ai creature history compresses stages, mutations, badges, case choices, and incidents;",
      "  chronicle derives 7/30/90-day course and generation comparison without writing state;",
      ...
      "  State: ~/.anti-ai/creature.json (schema v16; schema v1-v15 migrate sequentially",
      ...
      "  fossils, evolution choices, turning-point cases, containment incidents, derived cultures and ingredient",
      "  references, companion bonds/discrete imprints/anomaly IDs, cabinet IDs, daily interaction IDs, expedition plans/events/effects/collection IDs, dose, traits,",
      "  ability/chromatic gains, privacy-safe metabolism/study IDs, visitor stay IDs/dates, events, and a",
      ...
      "",
      color("1", "Containment habitat"),
      "  anti-ai creature habitat renders one read-only cohabitation scene for the",
      "  creature and its companion. --full expands sealed incidents; --json exposes",
      "  the same derived snapshot without saving it.",
      "  One deterministic ecological incident appears per seven experience days.",
      "  Fifteen deterministic living-scene archetypes combine Ecology, specimen posture, cohabitation, and recent traces without rerolls.",
      "  POLLUTION, CLARITY, and PARADOX have equal pools: 30 incidents, 24 relationship",
      "  diagnoses, 24 decorations, and 36 duo titles across all routes.",
      "  Habitat output can change scenery, relationship copy, collection discoveries, and the three-slot consequence cabinet;",
      "  it cannot change creature or companion numbers or reward additional Token use.",
      "  A saved foreign specimen may explicitly occupy one visitor bay. Natural-date cohabitation",
      "  derives a route-balanced diagnosis, bulletin, and joint exhibit without changing growth.",
      "",
      ...
      "  briefing adds a privacy-safe daily broadcast; specimen, wanted, fossil, encounter, prognosis, culture, companion, habitat, expedition, and dossier add collection cards.",
      ...
      "  CLI SVGs use stdout; a confirmed TUI preview creates only its new local file.",
      "  If the current directory is not writable, TUI exports fall back to ~/.anti-ai/exports. Nothing is uploaded.",
      "",
  ...
  const lines = [
    ...
    "  成长平衡 v2 使用此前最多 28 个非零用量日的中位数；AI 清醒日不会压低基线。",
    "  高用量增加 1–3 点污染性，低用量增加 1–2 点清醒性，AI 清醒日增加 3 点清醒性。",
    "  污染剂量达到 75+ 时至少增加 1 点污染性，但不会额外增加能力点或加速阶段。",
    "  终身污染性与清醒性继续留在病历；当前人格由最近 28 个阅历日形成未定型、污染型、清醒型或矛盾型。",
    ...
    "  36 项成就平均分为罪证章、戒断章、悖论章三类；可重复成就按行为次数成长为三级，",
    "  不使用精确 Token 作为等级条件。anti-ai codex 从 schema v16 派生 134 项固定收藏、动态/外来标本、化石、已封存病例切片、培养物、伴生异物和事故报告。",
    "  十二组路线对等的病理星图从这些发现派生；四段病程与隐藏传奇诊断只解锁展示内容。",
    "  固定收藏达到 34/67/101/134 项里程碑后，可增加 28 种只影响展示的冠层或外骨骼，不改变基础指纹。",
    "  基础标本容量仍为 204,374,016；29 种馆藏表型形成 5,926,846,464 种理论展示形态。",
    "  人类输出的锁定项保持 ???，JSON 提供稳定 ID 和发现日期。",
    "  TUI 可把最多 3 项已发现收藏放入后果陈列柜；每个已结算日最多记录 1 次观察和 1 次接触。",
    "  陈列与轻互动只改变展示和叙事，不增加能力、阅历、稀有率或 Token 收益。",
    ...
    "  每 14 个阅历日最多出现一个转折病例，从 24 个本地病例骨架中确定性选择。",
    "  污染、清醒、悖论三条路线各有收益与代价；未处理病例不会堆积成待办列表。",
    "  每 7 个阅历日最多出现一个收容事故；高消耗、克制使用和 AI 清醒日等速推进。",
    "  紧急隔离、继续观察、允许共振都会封存收益与代价，延迟后果会在 3 个阅历日后结算；",
    "  响应不会增加能力、阅历或 Token 奖励，未处理事故也不会堆积成待办。",
    "  每个本地自然日最多开启一局十格收容远征，不依赖结算；机会不累计，退出可恢复，放弃会消耗当天机会且不能重抽。",
    "  四个目的地会封存确定性事件序列；Token 量不改变局数、格数、事件、遗物概率或稀有度。",
    "  远征为图鉴增加 24 件固定遗物和 12 项固定成就；每局永久能力微调最多一次，通常为 ±1，极稀有命名事件为 ±2。",
    "  Token 代谢门诊只描述 usage metadata 的相关性观察，不评价生产力、因果关系、个人能力或健康。",
    "  相对规则使用此前最多 14 个活跃日，至少 3 个可比样本；7、14、30 天被动研究按自然日推进。",
    "  漏日不会清零、惩罚或延长课题；研究印章不会增加能力、阅历、稀有率或收藏进度，也不提供 Token 奖励。",
    "  anti-ai creature history 压缩阶段、突变、徽章、病例选择和事故；chronicle 只读派生 7/30/90 天病程与世代对照；--full 展开隐私安全的逐日用量带。",
    "  prognosis 使用“主导/可能/潜伏”预演三条可解释方向，不伪造精确概率。",
    "  状态文件：~/.anti-ai/creature.json（schema v16；schema v1-v15 逐版本本地迁移并保留原始备份）",
    "  只保存用量带、派生生态点、基因/部件 ID、成就、化石、进化选择、转折病例、收容事故、培养物及素材引用、伴生绑定/离散印记/异常 ID、访客共处 ID/日期、陈列条目 ID、每日轻互动 ID、远征计划/事件/变化/收藏 ID、代谢诊断/研究 ID、污染剂量、性状、能力与异色加点、事件和本地 seed；",
    ...
    "",
    color("1", "收容生态舱"),
    "  anti-ai creature habitat 以单屏场景展示异变体与伴生物的共居状态；",
    "  --full 展开已封存事件，--json 输出相同的只读派生快照，不写入档案。",
    "  每 7 个阅历日确定性派生一次生态事件。污染、清醒、悖论路线拥有同等规模的池：",
    "  15 种确定性活体场景会组合生态人格、标本姿态、共居关系与近期痕迹，重复查看不能重抽。",
    "  共 30 个事件、24 种关系诊断、24 件布景和 36 个双生称号。",
    "  生态舱只改变场景、关系文案与收藏发现；不会改变异变体或伴生物数值，",
    "  也不会奖励更多 Token 消耗。",
    "  已保存外来标本可显式入住一个访客位；共处按自然日派生路线对等的诊断、短讯和共同展品，",
    "  不改变本体或伴生物成长。",
    "",
    color("1", "活体病历"),
    ...
    "  briefing 提供隐私安全的每日播报；specimen、wanted、fossil、encounter、prognosis、culture、companion、habitat、expedition、dossier 提供收藏卡。",
    "  creature habitat、codex 和所有分享卡都是只读快照，不会创建或推进异变体档案。",
    "  creature export 与 encounter 只交换派生外观 ID，全部混合都在本地完成。",
    "  所有卡片都不包含对话、路径、模型名或精确 Token，也不包含来源名和请求数；CLI SVG 写入标准输出，TUI 确认后只新建预览文件；当前目录不可写时改存 ~/.anti-ai/exports，均不会上传。",
    "",
