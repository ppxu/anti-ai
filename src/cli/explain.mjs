  ...
  const sections = {
    ...
    creature: {
      zh: [
        ...
        "每个新阅历日最多开启一局十格收容远征；机会不累计，退出可恢复，放弃不可重抽。",
      ...
      en: [
        ...
        "Each new experience day offers at most one ten-cell containment expedition; opportunities do not stack, exits can resume, and abandonment cannot reroll.",
    ...
    privacy: {
      zh: [
        ...
        "creature habitat、codex 和所有 SVG 分享卡都是只读快照；CLI 卡片只写入标准输出，TUI 确认后只新建预览过的 SVG。",
      ...
      en: [
        ...
        "Creature habitat, Codex, and every SVG card are read-only snapshots; CLI cards use stdout, while confirmed TUI exports create only the previewed SVG.",
...
function runExplain(lang = "zh", topic = undefined) {
  ...
  if (lang === "en") {
    const lines = [
      ...
      "  Each new experience day offers at most one ten-cell containment expedition;",
      "  opportunities do not stack; skipping has no penalty, but past opportunities expire.",
      ...
      "  CLI SVGs use stdout; a confirmed TUI preview creates only its new local file. Nothing is uploaded.",
      "",
  ...
  const lines = [
    ...
    "  每个新阅历日最多开启一局十格收容远征，机会不累计；退出可恢复，放弃会消耗机会且不能重抽。",
    ...
    "  所有卡片都不包含对话、路径、模型名或精确 Token，也不包含来源名和请求数；CLI SVG 写入标准输出，TUI 确认后只新建预览文件，均不会上传。",
    "",
