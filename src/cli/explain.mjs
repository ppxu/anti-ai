...
function runExplain(lang = "zh", topic = undefined) {
  ...
  if (lang === "en") {
    const lines = [
      ...
      "  anti-ai codex derives 68 fixed entries plus private, foreign, fossil, selected case, culture, companion, and incident-report specimens from schema v11.",
      ...
      "  State: ~/.anti-ai/creature.json (schema v11; schema v1-v10 migrate sequentially",
      ...
      "  Habitat output can change scenery, relationship copy, and collection discoveries;",
  ...
  const lines = [
    ...
    "  不使用精确 Token 作为等级条件。anti-ai codex 从 schema v11 派生 68 项固定收藏、动态/外来标本、化石、已封存病例切片、培养物、伴生异物和事故报告。",
    ...
    "  状态文件：~/.anti-ai/creature.json（schema v11；schema v1-v10 逐版本本地迁移并保留原始备份）",
    "  只保存用量带、派生生态点、基因/部件 ID、成就、化石、进化选择、转折病例、收容事故、培养物及素材引用、伴生绑定/离散印记/异常 ID、污染剂量、性状、能力与异色加点、事件和本地 seed；",
