...
function runExplain(lang = "zh", topic = undefined) {
  ...
  if (lang === "en") {
    const lines = [
      ...
      "  anti-ai codex derives 134 fixed entries plus private, foreign, fossil, selected case, culture, companion, and incident-report specimens from schema v14.",
      ...
      "  State: ~/.anti-ai/creature.json (schema v14; schema v1-v13 migrate sequentially",
  ...
  const lines = [
    ...
    "  不使用精确 Token 作为等级条件。anti-ai codex 从 schema v14 派生 134 项固定收藏、动态/外来标本、化石、已封存病例切片、培养物、伴生异物和事故报告。",
    ...
    "  状态文件：~/.anti-ai/creature.json（schema v14；schema v1-v13 逐版本本地迁移并保留原始备份）",
    "  只保存用量带、派生生态点、基因/部件 ID、成就、化石、进化选择、转折病例、收容事故、培养物及素材引用、伴生绑定/离散印记/异常 ID、陈列条目 ID、每日轻互动 ID、远征计划/事件/变化/收藏 ID、污染剂量、性状、能力与异色加点、事件和本地 seed；",
